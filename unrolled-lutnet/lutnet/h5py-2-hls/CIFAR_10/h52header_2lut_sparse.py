...
if __name__ == "__main__":
    ...
    bl = h5py.File("pretrained_network_2lut.h5", 'r')
    ...
    bl_rand_map = np.array(bl["model_weights"]["binary_conv_1"]["binary_conv_1"]["rand_map:0"])
    ...
    bl_rand_map = np.array(bl["model_weights"]["binary_conv_2"]["binary_conv_2"]["rand_map:0"])
    ...
    bl_rand_map = np.array(bl["model_weights"]["binary_conv_3"]["binary_conv_3"]["rand_map:0"])
    ...
    bl_rand_map = np.array(bl["model_weights"]["binary_conv_4"]["binary_conv_4"]["rand_map:0"])
    ...
    bl_rand_map = np.array(bl["model_weights"]["binary_conv_5"]["binary_conv_5"]["rand_map:0"])
    ...
    bl_rand_map = np.array(bl["model_weights"]["binary_conv_6"]["binary_conv_6"]["rand_map:0"])
    ...
    for layer_id in range(9):
        ...
        if layer_id!=5:  # first layer: fxp inputs and binary weights
            ...
            weights_per_act = 8 # weights_per_act = #_of_bits_per_act x 2 ^ #_of_lut_inputs
                ...
                for tm in range(nfilters):
                    for ti, ele in enumerate(pm_flat[tm]):
                        if ele==1:
                            ...
                            v0.write('(' + str(int(mat_flat[4][tm][ti])) + ' & in_V[' + str(ti) + '] & in_1_V[' + str(ti) + ']) | ')
                            v0.write('(' + str(int(mat_flat[5][tm][ti])) + ' & in_V[' + str(ti) + '] & ~in_1_V[' + str(ti) + ']) | ')
                            v0.write('(' + str(int(mat_flat[6][tm][ti])) + ' & ~in_V[' + str(ti) + '] & in_1_V[' + str(ti) + ']) | ')
                            v0.write('(' + str(int(mat_flat[7][tm][ti])) + ' & ~in_V[' + str(ti) + '] & ~in_1_V[' + str(ti) + ']);\n')
                ...
                for tm in range(nfilters):
                    for ti, ele in enumerate(pm_flat[tm]):
                        if ele==1:
                            ...
                            v1.write('(' + str(int(mat_flat[0][tm][ti])) + ' & in_V[' + str(ti) + '] & in_1_V[' + str(ti) + ']) | ')
                            v1.write('(' + str(int(mat_flat[1][tm][ti])) + ' & in_V[' + str(ti) + '] & ~in_1_V[' + str(ti) + ']) | ')
                            v1.write('(' + str(int(mat_flat[2][tm][ti])) + ' & ~in_V[' + str(ti) + '] & in_1_V[' + str(ti) + ']) | ')
                            v1.write('(' + str(int(mat_flat[3][tm][ti])) + ' & ~in_V[' + str(ti) + '] & ~in_1_V[' + str(ti) + ']);\n')
        ...
        if layer_id==0:
            ...
            with open('../src/weights.h', 'a') as f:
                fold = (word_length-1)/32 + 1
                f.write("const ap_uint<32> " + "pruning_mask_" + layer_type + str(layer_id+1) + "_" + str(weight_id+1) + "["+str(nfilters*fold) + "] = {")
                bin_append = 0
