...
cd ../../..
python Binary.py ${dataset} True False False True False True False ${retrainEpochs} > output.txt
mkdir -p models/${dataset}/pruned_lutnet
cp models/${dataset}/2_residuals.h5 models/${dataset}/pruned_lutnet/pruned_lutnet_${id}_FP32.h5
cp output.txt models/${dataset}/pruned_lutnet/pruned_lutnet_${id}_FP32.txt

cp models/${dataset}/2_residuals.h5  models/${dataset}/pretrained_bin.h5
