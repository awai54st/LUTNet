...
void XNORARRAY(ap_uint<MACS> in, ap_uint<((MACS-1)/32+1)*32> *weight_0, ap_uint<((MACS-1)/32+1)*32> *pruning_mask_0, ap_uint<MACS> *lut_out){
//#pragma HLS inline off
        for (int tm = 0; tm < M; tm++) {
        #pragma HLS UNROLL
            lut_out[tm] = (~(in ^ weight_0[tm].range(MACS-1,0))) & pruning_mask_0[tm].range(MACS-1,0);
...
ap_uint<PRECISION_REB*M> XNOR_TM(ap_uint<MACS> in[PRECISION_REB][TN], const ap_uint<32> weightMem[M/TM][TN][TM][(MACS-1)/32+1], const ap_fixed<PopCountWidth, PopCountIntWidth> *thresh, const ap_fixed<PopCountWidth, PopCountIntWidth> *alpha, const ap_fixed<PopCountWidth, PopCountIntWidth> *next_layer_means, const ap_uint<32> pruning_mask[M/TM][(MACS-1)/32+1], bool pt) {
        ...
        ap_uint<max_fold*32> weightMem_0[M/TM];
        #pragma HLS ARRAY_PARTITION variable=weightMem_0 complete dim=1
        ap_uint<max_fold*32> pruning_mask_0[M/TM];
        #pragma HLS ARRAY_PARTITION variable=pruning_mask_0 complete dim=1
        ...
        #pragma HLS UNROLL
            ...
            #pragma HLS UNROLL
                pruning_mask_0[tm].range(32*(fold+1)-1, 32*fold) = pruning_mask[tm][fold];
            }
        }
        ...
        for (int tile_m = 0; tile_m < TM; tile_m++){
            ...
            #pragma HLS PIPELINE II=1
                ...
                #pragma HLS UNROLL
                    for (int fold = 0; fold < max_fold; fold++){
                    #pragma HLS UNROLL
                        weightMem_0[tm].range(32*(fold+1)-1, 32*fold) = weightMem[tm][tile_n][tile_m][fold];
                    }
                }

                ap_uint<MACS> lut_out_b0[M/TM];
                #pragma HLS ARRAY_PARTITION variable=lut_out_b0 complete dim=1
                XNORARRAY<MACS, M/TM>(in[0][tile_n], weightMem_0, pruning_mask_0, lut_out_b0);
        
                ap_uint<MACS> lut_out_b1[M/TM];
                #pragma HLS ARRAY_PARTITION variable=lut_out_b1 complete dim=1
                XNORARRAY<MACS, M/TM>(in[1][tile_n], weightMem_0, pruning_mask_0, lut_out_b1);
...
void LUTNET_REBMV_TM(
  ...
  const ap_uint<32> pruning_mask[M/TM][(N*K*K/TN-1)/32+1]
)
{
...
//                            tmp_in_reb[tb].range(t_mac, t_mac) = tmp_in.range(t_mac*PRECISION_REB+tb, t_mac*PRECISION_REB+tb); // tmp_in_reb[0] is LSB
      ...
      tmp_out = XNOR_TM<PRECISION_REB, N*K*K/TN, PopCountWidth, PopCountIntWidth, M, TN, TM> (tmp_in_reb, weightMem, thresh, alpha, next_layer_means, pruning_mask, pt);
