...
ap_uint<OutputWidth*M> XNOR_TM_NOTH(ap_uint<MACS> in[PRECISION_REB][TN], const ap_uint<32> weightMem[M/TM][TN][TM][(MACS-1)/32+1], const ap_fixed<PopCountWidth, PopCountIntWidth> *alpha, const ap_uint<32> pruning_mask[M/TM][(MACS-1)/32+1], bool pt) {
...
//                            tmp_in_reb[tb].range(t_mac, t_mac) = tmp_in.range(t_mac*PRECISION_REB+tb, t_mac*PRECISION_REB+tb); // tmp_in_reb[0] is LSB
      ...
      tmp_out = XNOR_TM_NOTH<PRECISION_REB, N*K*K/TN, PopCountWidth, PopCountIntWidth, OutputWidth, M, TN, TM> (tmp_in_reb, weightMem, alpha, pruning_mask, pt);
