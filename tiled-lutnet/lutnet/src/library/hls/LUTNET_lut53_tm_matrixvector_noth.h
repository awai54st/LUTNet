...
ap_uint<OutputWidth*M> LUT_TM_NOTH(ap_uint<MACS> in[PRECISION_REB][TN], ap_uint<MACS> in_1[PRECISION_REB][TN], const ap_uint<32> weightMem0[M/TM][TN][TM][(MACS-1)/32+1], const ap_uint<32> weightMem1[M/TM][TN][TM][(MACS-1)/32+1], const ap_uint<32> weightMem2[M/TM][TN][TM][(MACS-1)/32+1], const ap_fixed<PopCountWidth, PopCountIntWidth> *alpha, const unsigned int *rand_map_0, bool pt) {
...
//                            tmp_in_reb[tb].range(t_mac, t_mac) = tmp_in.range(t_mac*PRECISION_REB+tb, t_mac*PRECISION_REB+tb); // tmp_in_reb[0] is LSB
      ...
      tmp_out = LUT_TM_NOTH<PRECISION_REB, N*K*K/TN, PopCountWidth, PopCountIntWidth, OutputWidth, M, TN, TM> (tmp_in_reb, tmp_in_1_reb, weightMem0, weightMem1, weightMem2, alpha, rand_map_0, pt);
