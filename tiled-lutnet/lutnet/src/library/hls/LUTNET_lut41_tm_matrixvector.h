...
void LUTARRAY(ap_uint<MACS> in, ap_uint<MACS> in_1, ap_uint<MACS> in_2, ap_uint<((MACS-1)/32+1)*32> *weight_0, ap_uint<MACS> *lut_out){
#pragma HLS inline off
        for (int tm = 0; tm < M; tm++) {
        #pragma HLS UNROLL
            lut_out[tm] = in ^ in_1 ^ in_2 ^ weight_0[tm].range(MACS-1,0);
...
ap_uint<PRECISION_REB*M> LUT_TM(ap_uint<MACS> in[PRECISION_REB][TN], ap_uint<MACS> in_1[PRECISION_REB][TN], ap_uint<MACS> in_2[PRECISION_REB][TN], const ap_uint<32> weightMem0[M/TM][TN][TM][(MACS-1)/32+1], const ap_fixed<PopCountWidth, PopCountIntWidth> *thresh, const ap_fixed<PopCountWidth, PopCountIntWidth> *alpha, const ap_fixed<PopCountWidth, PopCountIntWidth> *next_layer_means, const unsigned int *rand_map_0, const unsigned int *rand_map_1, bool pt) {
...
//                            tmp_in_reb[tb].range(t_mac, t_mac) = tmp_in.range(t_mac*PRECISION_REB+tb, t_mac*PRECISION_REB+tb); // tmp_in_reb[0] is LSB
      ...
      tmp_out = LUT_TM<PRECISION_REB, N*K*K/TN, PopCountWidth, PopCountIntWidth, M, TN, TM> (tmp_in_reb, tmp_in_1_reb, tmp_in_2_reb, weightMem0, thresh, alpha, next_layer_means, rand_map_0, rand_map_1, pt);
